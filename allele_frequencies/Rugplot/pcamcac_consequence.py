...
consequence_color = {
    'missense_variant': 'olivedrab',
    'missense_variant&splice_region_variant': 'indigo',
    'missense_variant&NMD_transcript_variant': 'maroon',
    'missense_variant&splice_region_variant&NMD_transcript_variant': 'red'
        }

color_cons = []
for a in cons:
    color_cons.append(consequence_color[a])
...
i=0
for l in reader:
    markers.append(CircleMarker(r=2.5, fill=color_cons[i]))
    i += 1
    for n in range(2,9):
...
rug = svgwrite.Drawing('pcamcac_consequence.svg')
pairs = []
