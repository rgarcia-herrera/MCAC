...
print "\t".join( ["Uploaded_variation", "Location", "Allele", "Gene", "Feature", "Feature_type",
                  ...
                  "Existing_variation",  'IMPACT', 'VARIANT_CLASS', 'SYMBOL', 'SYMBOL_SOURCE', 'STRAND', 'ENSP',
...
for l in csvr:
    if not l[0].startswith('#'):
        ...
        if l[6] == 'missense_variant' \
           or extras.get('IMPACT','-') == 'HIGH' \
           or (extras.get('CLIN_SIG','-').find("pathogenic") != -1) \
           or l[12] == '-' \
           or extras.get('BIOTYPE','-') == 'promoter' :
